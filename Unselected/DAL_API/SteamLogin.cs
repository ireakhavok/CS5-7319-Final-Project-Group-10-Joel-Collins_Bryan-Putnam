...
namespace DAL_API
{
    ...
    public class SteamLogin
    {
        ...
        /// <remarks>If you want to know how the request method works, use: <see cref="SteamLogin.Request"/></remarks>
        ...
        private string EncryptPassword(RsaParameters rsaParam)
        {
            ...
            var encryptedNumberByteArray = encryptedNumber.ToByteArray();
            var reversed = encryptedNumberByteArray.Reverse();

            var encryptedString = Convert.ToBase64String(reversed.ToArray());

            return encryptedString;
        }
        //https://stackoverflow.com/questions/43553513/c-sharp-steam-webrequests
        //https://github.com/Jessecar96/SteamBot/blob/master/SteamTrade/SteamWeb.cs#L172
        ...
        public Dictionary<string, string> DoLoginEncryptedAlready(string username, string password, string timestamp, string steamGuardText = "", bool emailAuth = false, bool twoFactorAuth = false)
        {
            ...
            do
            {
                ...
                bool steamGuard = loginJson != null && (loginJson.emailauth_needed || loginJson.requires_twofactor);

                string time = Uri.EscapeDataString(timestamp);
                ...
                data = new NameValueCollection { { "password", encrypted }, { "username", username } };

                // Captcha Check.
                string capText = "";
                if (captcha)
                {
                    result.Add("CaptchaNeeded","true");
                    return result;
                    ...
                    ////TODO this may have to change for web type result
                ...
                data.Add("loginfriendlyname", "GameButler");
                ...
                if (steamGuard)
                {
                    ...
                    if (loginJson.emailauth_needed)
                    {
                        SteamUserID = steamGuardId = loginJson.emailsteamid;
                        if (!(result.ContainsValue(SteamUserID)))
                        {
                            result.Add("steamid", SteamUserID);
                            
                        }
                        result.Add("EmailAuthNeeded", "true");
                    }
                    else
                    {
                        result.Add("EmailAuthNeeded", "false");
                    }
                    if (loginJson.requires_twofactor)
                    {
                        result.Add("TwoFactorNeeded", "true");
                    }
                    else
                    {
                        result.Add("TwoFactorNeeded", "false");
                    }
                    return result;
                ...
                if (twoFactorAuth)
                {
                    data.Add("twofactorcode", steamGuardText);
                }
                ...
                using (HttpWebResponse webResponse = Request("https://steamcommunity.com/login/dologin/", "POST", data, false))
                {
                    ...
                    using (StreamReader reader = new StreamReader(stream))
                    {
                        ...
                        {
                            var added = result.TryAdd("success",  "true");
                            if (!added)
                                result["success"] = "true";
                        }
                        ...
                        foreach (Cookie cookie in cookieCollection)
                        {
                            ...
                            {
                                Cookie steamIDcookie = cookieCollection.FirstOrDefault(n => n.Name == "steamLoginSecure");
                                Regex re = new Regex("([0-9]*)%");
                                Match match1 = re.Match(steamIDcookie.Value);
                                SteamUserID = match1.Groups[1].Value;
                                result.Add("steamid", SteamUserID);
                                var added = result.TryAdd("success", "true");
                                if (!added)
                                    result["success"] = "true";
            ...
            if (loginJson.success)
            {
                using (HttpWebResponse webResponse = Request("https://steamcommunity.com/", "GET"))
                {
                    var stream = webResponse.GetResponseStream();
                    using (StreamReader reader = new StreamReader(stream))
                    {
                ...
                {
                    _cookies.Add(cookie);
                    if (!result.ContainsKey(cookie.Name))
                        result.Add(cookie.Name, cookie.Value);
                }
        ...
        /// <param name="password">Your Steam password.</param>
        /// <returns>A bool containing a value, if the login was successful.</returns>
        public Dictionary<string, string> DoLogin(string username, string password, string steamGuardText = "", bool emailAuth = false, bool twoFactorAuth = false)
        {
            Dictionary<string, string> result = new Dictionary<string, string>();
            //{
            ...
            do
            {
                ...
                {
                    SteamUserID = steamGuardId = loginJson.emailsteamid;
                    var added = result.TryAdd("steamid", SteamUserID);
                    if (!added)
                        result["steamid"] = SteamUserID;
                    ...
                    using (StreamReader reader = new StreamReader(stream))
                    {
                        ...
                        foreach (Cookie cookie in cookieCollection)
                        {
                            ...
                            if (cookie.Name == "steamLoginSecure")
                            {
                                ...
                                SteamUserID = match1.Groups[1].Value;
                                result.Add("steamid", SteamUserID);
                                result.Add("steamLoginsecure", cookie.Value);
                                var added = result.TryAdd("success", "true");
                                if (!added)
                                    result["success"] = "true";
                            }
            ...
            //SteamUserID = loginJson.emailsteamid;
            ...
            if (loginJson.success)
            {
                ...
                ////result["steamid"] = SteamUserID;
                //result["steamid"] = loginJson.transfer_parameters.steamid;

                //result["token_secure"] = loginJson.transfer_parameters.token_secure;
                //result["auth"] = loginJson.transfer_parameters.auth;
                //result["remember_login"] = loginJson.transfer_parameters.remember_login.ToString();
        ...
        public void Authenticate(System.Collections.Generic.IEnumerable<Cookie> cookies)
        {
            ...
            foreach (var cookie in cookies)
            {
                ...
                else if (cookie.Name == "steamLoginSecure")
                {
                    ...
                    Regex re = new Regex("([0-9]*)%");
                    Match match1 = re.Match(tokenSecure);
                    token = match1.Groups[1].Value;
                }
        ...
        /// </summary>
        /// <returns>true if cookies are correct; false otherwise</returns>//https://api.steampowered.com/ISteamUserAuth/AuthenticateUser/v1/
        ...
        public bool AuthenticateUser(string steamid, string sessionkey, string encrypted_loginkey)
        {
            bool result = false;
            NameValueCollection data = new NameValueCollection { { "steamid", steamid }, { "sessionkey", sessionkey }, { "encrypted_loginkey", encrypted_loginkey } };
            //data.Add("rsatimestamp", time);

            // Sending the actual login.
            using (HttpWebResponse webResponse = Request("https://partner.steam-api.com/ISteamUserAuth/AuthenticateUser/v1/",
                "POST", data, false))
            {
                CookieCollection cookieCollection;
                ...
                {
                    string json = reader.ReadToEnd();
                    AuthResult loginJson = JsonConvert.DeserializeObject<AuthResult>(json);
                    cookieCollection = webResponse.Cookies;
                }
            }
            return true;
    ...
    /// <summary>
    /// Class to Deserialize the json response strings of the getResKey request. See: <see cref="SteamLogin.DoLogin"/>
    /// </summary>
    ...
    /// <summary>
    /// Class to Deserialize the json response strings after the login. See: <see cref="SteamLogin.DoLogin"/>
    /// </summary>
    //[SuppressMessage("ReSharper", "InconsistentNaming")]
    public class SteamResult
    {
        ...
        public transferParameters? transfer_parameters
        {
            get; set;
